  ...
  methods: {
    ...
    formatMatchDate: function formatMatchDate(dateStr) {
      ...
      if (dateStr.includes('T') || dateStr.length > 10 && dateStr.includes(':')) {
        ...
        var bd = new Date(utc.getTime() + 6 * 60 * 60 * 1000);
        return "".concat(days[bd.getUTCDay()], ", ").concat(months[bd.getUTCMonth()], " ").concat(bd.getUTCDate());
      }
      // Date-only strings: DD/MM/YYYY or YYYY-MM-DD
      var year, month, day;
      if (dateStr.includes('/')) {
        var _dateStr$split$map = dateStr.split('/').map(Number);
        var _dateStr$split$map2 = _slicedToArray(_dateStr$split$map, 3);
        day = _dateStr$split$map2[0];
        month = _dateStr$split$map2[1];
        year = _dateStr$split$map2[2];
      } else {
        var _dateStr$split$map3 = dateStr.split('-').map(Number);
        var _dateStr$split$map4 = _slicedToArray(_dateStr$split$map3, 3);
        year = _dateStr$split$map4[0];
        month = _dateStr$split$map4[1];
        day = _dateStr$split$map4[2];
      }
