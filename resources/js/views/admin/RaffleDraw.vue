        ...
        <div class="raffle-layout">
            ...
            <div class="raffle-left">
                <div class="card-header">
                    ...
                    <button class="btn-random" @click="randomDraw">🎲 Random Draw</button>
                </div>
...
export default {
    name: 'AdminRaffle',
    data() {
        return { eligible: [], history: [], selectedUserId: null, minPoints: 0, prizePoints: 0, notes: '', randomWinner: null };
    },
    computed: {
        selectedUser() { return this.eligible.find(u => u.id === this.selectedUserId); },
    },
    mounted() { this.fetchEligible(); this.fetchHistory(); },
    methods: {
        async fetchEligible() {
            const { data } = await this.$http.get('/api/admin/raffle/eligible', { params: { min_points: this.minPoints } });
            this.eligible = data.data;
        },
        async fetchHistory() { const { data } = await this.$http.get('/api/admin/raffle/history'); this.history = data.data; },
        async randomDraw() {
            try {
                const { data } = await this.$http.post('/api/admin/raffle/random', { min_points: this.minPoints });
        ...
        async confirmDraw() {
            try {
                await this.$http.post('/api/admin/raffle/draw', {
                    user_id: this.selectedUserId, prize_points: this.prizePoints, notes: this.notes,
                });
                this.$toaster.success('Winner confirmed!');
                this.selectedUserId = null; this.prizePoints = 0; this.notes = '';
                this.fetchHistory();
            } catch (e) { this.$toaster.error('Failed.'); }
        },
    },
};
</script>
