<template>
  <div>
    <div class="modal fade" id="edit-delivery" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
export default {
  ...
  mounted() {
    bus.$on('edit-delivery', (row) => {
      this.title = 'Update Delivery';
      this.buttonText = "Save";
      this.customerName = row.CustomerName;
      this.customerMobile = row.CustomerMobile;
      this.customerAddress = row.CustomerAddress;
      this.deliveryAddress = row.DeliveryAddress;
      this.invoiceNumber = row.InvoiceID.substring(12, 16);
      this.riderId = row.RiderID;
      this.specialNotes = row.SpecialInstruction;
      this.deliveryId = row.DeliveryID;
      this.amount = row.OrderAmount;
      this.invoicePrefix = row.InvoiceID.substring(0, 12);;
      this.isDisabled = true;
      this.customerId = row.CustomerID
      $("#edit-delivery").modal("toggle");
    })
  },
  methods: {
    resetCustomer() {
    ...
    onSubmit() {
      ...
      this.axiosPost('update_delivery', {
